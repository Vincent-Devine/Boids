...
[BurstCompile]
public partial struct ECS_SpawnerSystem : ISystem
{
    EntityQuery m_BoidQuery;

    public void OnCreate(ref SystemState state)
    {
        m_BoidQuery = new EntityQueryBuilder(Allocator.Temp).WithAll<ECS_Boid, LocalTransform>().Build(ref state);
    }
    ...
    public void OnUpdate(ref SystemState state)
    {
        ...
        foreach (var (spawner, spawnerLocalToWorld, entity) in
                 SystemAPI.Query<RefRO<ECS_Spawner>, RefRO<LocalToWorld>>()
                     .WithEntityAccess())
        {
            var boidEntities =
            ...
            state.EntityManager.Instantiate(spawner.ValueRO.Prefab, boidEntities);
            ...
            state.Dependency = setBoidLocalToWorldJob.Schedule(spawner.ValueRO.Count, 64, state.Dependency);
