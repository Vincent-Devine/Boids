...
public partial struct BoidSystem : ISystem
{
    ...
    {
        var boidQuery = SystemAPI.QueryBuilder().WithAll<ECS_Boid>().WithAllRW<LocalToWorld>().Build();
        var obstacleQuery = SystemAPI.QueryBuilder().WithAll<ECS_Obstacle, LocalToWorld>().Build();
        var obstacleCount = obstacleQuery.CalculateEntityCount();
        
        var world = state.WorldUnmanaged;
        state.EntityManager.GetAllUniqueSharedComponents(out NativeList<ECS_Boid> uniqueBoidTypes, world.UpdateAllocator.ToAllocator);
        ...
        foreach (var boidSettings in uniqueBoidTypes)
        {
            ...
            var mergeCellsBarrierJobHandle = JobHandle.CombineDependencies(initialCellBarrierJobHandle, initialObstacleJobHandle);
...
// 2) find the nearest obstacle to each boid cell
...
partial struct SteerBoidJob : IJobEntity
{
    ...
    [ReadOnly] public NativeArray<float3> ObstaclePositions;
    public ECS_Boid CurrentBoidVariant;
    ...
    void Execute([EntityIndexInQuery] int entityIndexInQuery, ref LocalToWorld localToWorld)
    {
        ...
        var normalHeading                     = math.normalizesafe(alignmentResult + separationResult);
